    ...
    return new Page('main')
        ...
        .add(new Page('LIGHTING')
            ...
            .add(new Page('LIGHTING_GI')
                ...
                .add(asInt('LIGHTING_GI_SAMPLES', 8, 12, 16, 24, 32).needsReload(false).build(16))
                .add(asFloatRange('LIGHTING_GI_RADIUS', 6.0, 2.0, 12.0, 0.5, false))
                .add(asFloatRange('LIGHTING_GI_STRENGTH', 1.0, 0.0, 2.0, 0.05, false))
                .add(asFloatRange('LIGHTING_GI_TEMPORAL', 0.9, 0.0, 0.99, 0.05, false))
                .add(asFloatRange('LIGHTING_GI_HISTORY_MAX', 16.0, 1.0, 32.0, 1.0, false))
                .build())
            .add(EMPTY)
            .add(new Page('LIGHTING_LPV')
                .add(asBool('LIGHTING_LPV_ENABLED', false, true))
                .add(asFloatRange('LIGHTING_LPV_RANGE', 32.0, 8.0, 96.0, 4.0, false))
                .add(asFloatRange('LIGHTING_LPV_STRENGTH', 0.5, 0.0, 2.0, 0.05, false))
                .add(asFloatRange('LIGHTING_LPV_TEMPORAL', 0.6, 0.0, 1.0, 0.05, false))
                .add(asFloatRange('LIGHTING_LPV_PROPAGATION', 0.25, 0.0, 1.0, 0.05, false))
                .add(asFloatRange('LIGHTING_LPV_MAX', 12.0, 1.0, 48.0, 1.0, false))
                .build())
            .add(EMPTY)
            .add(new Page('LIGHTING_AMBIENT')
                .add(asFloatRange('LIGHTING_AMBIENT_BRIGHTNESS', 0.02, 0.0, 0.5, 0.01, false))
                .add(asFloatRange('LIGHTING_AMBIENT_RED', 1.0, 0.0, 2.0, 0.05, false))
                .add(asFloatRange('LIGHTING_AMBIENT_GREEN', 1.0, 0.0, 2.0, 0.05, false))
                .add(asFloatRange('LIGHTING_AMBIENT_BLUE', 1.0, 0.0, 2.0, 0.05, false))
                .build())
            .add(EMPTY)
